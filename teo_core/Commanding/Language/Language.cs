...
namespace TEO.Commanding.Language
{
    ...
    public partial class Language: IFactory<Input, IBatchable>
    {
        ...
        public static IBatchable Create_Display(Input item, ev.Environment env)
        {
            ...
            if (!item.TrySet(out adst, 2)) throw new ArgumentException("Второй аргумент, который должен описывать способ отображения, отуствует");
        ...
        public static IBatchable Create_Set(Input item, ev.Environment env)
        {
            ...
            if (!item.TrySet(out avar, 1)) throw new ArgumentException("Первый аргумент, который должен быть переменной отсутствует");
